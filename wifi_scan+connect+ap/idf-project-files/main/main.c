/**
 * @file main.c
 * @brief This program demonstrates how to configure the ESP32 to operate in both Station (STA) and Access Point (AP) modes simultaneously.
 *        It connects to a Wi-Fi network in STA mode and, upon successful connection, starts an Access Point (AP) with a configurable SSID and password.
 *        The program logs the Wi-Fi connection and Access Point setup statuses, and handles Wi-Fi events such as connecting, disconnecting, and IP address assignment.
 * 
 ...
 * 2. Set up Wi-Fi in station mode to connect to a specified network.
 * 3. On successful connection, configure and start an Access Point (AP).
 * 4. Handle Wi-Fi events (connection, disconnection, IP obtained) and switch between modes.
 * 
 * @functions
 * - wifi_event_handler(): Handles Wi-Fi events such as connection and disconnection.
 * - wifi_init_sta(): Initializes Wi-Fi in station mode and connects to the network.
 * - wifi_init_ap(): Initializes Wi-Fi in Access Point mode after connecting to the network.
 * - app_main(): The main function where initialization and event handling occur.
 */
