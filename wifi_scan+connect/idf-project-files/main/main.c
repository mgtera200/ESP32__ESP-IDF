/**
 * @file main.c
 * @brief This program demonstrates how to connect the ESP32 to a Wi-Fi network
 *        using FreeRTOS and the ESP-IDF Wi-Fi API. The program handles Wi-Fi
 *        events (such as connecting, disconnecting, and obtaining an IP address)
 *        and logs the connection status and IP address when successfully connected.
 * 
 ...
 * 3. Handle Wi-Fi connection events (start, disconnect, IP obtained).
 * 
 * @functions
 * - wifi_event_handler(): Event handler for handling Wi-Fi events.
 * - wifi_init_sta(): Initializes Wi-Fi in station mode and connects to the network.
 * - app_main(): Main function where Wi-Fi initialization and event handling are executed.
 */
